    ...
    <nav className={`fixed top-0 left-0 right-0 z-[100] transition-all duration-500 ${
      ...
      <div className="container mx-auto px-6 flex justify-between items-center">
        <Link to="/" className="flex items-center gap-4 md:gap-5 group shrink-0 relative z-10">
          <div className={`shrink-0 w-11 h-11 md:w-13 md:h-13 flex items-center justify-center transition-all duration-500 ${isScrolled ? 'bg-primary' : 'bg-white shadow-lg shadow-black/10'}`}>
             <span className={`${isScrolled ? 'text-white' : 'text-primary'} font-serif font-black text-2xl md:text-3xl`}>H</span>
          </div>
        ...
        <button className={`lg:hidden ${isScrolled ? 'text-primary' : 'text-white'}`}>
          <Search size={24} />
        </button>
      </div>
    </nav>
